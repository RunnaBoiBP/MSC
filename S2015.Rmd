...
>[http://www.dakstats.com/WebSync/Pages/Conference/ConferenceSchedule.aspx?association=10&sg=MBB&sea=NAIMBB_2015&conference=NAIMBB1_MS](http://www.dakstats.com/WebSync/Pages/Conference/ConferenceSchedule.aspx?association=10&sg=MBB&sea=NAIMBB_2015&conference=NAIMBB1_MS)
...
page <- read_html("/home/GEORGETOWNCOLLEGE.EDU/bpeterson/MSCBballShiny/DakStatsPages/MSCbball1516.html")
```
...
for ( i in seq(1, 2*n-1, by = 2) ) {
  id[c(i, i + 1)] <- i
  team[i] <- confGames$awayTeam[i]
  team[i + 1] <- confGames$homeTeam[i]
  status[i] <- "Away"
  status[i + 1] <- "Home"
}
...
df$SeasonChar <- "15-16"
S2015 <- df
save(S2015,file="data/S2015.rda")
```
